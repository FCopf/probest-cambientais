...
```{r echo = F}
n_amostra = 12
kableExtra::kbl(head(mun, n_amostra)) %>% 
  kable_paper("striped", full_width = F) %>%
  row_spec(c(4,5,6,7), bold = T, color = "white", background = "#D7261E")
```

Após entrevistar $`r n`$ pessoas selecionadas *ao acaso* de uma lista da moradores da cidade, é construída a tabela acima com três colunas: `Entrevistado` (sequência numérica do primeiro ao último respondente), `Opinião` e `Moradia`. 

Estão descritos acima os resultados das promeiras `r n_amostra` entrevistas, onde é possível ver ao menos uma combinação de todas as possíveis respostas. O entrevistado pode ser: i) `A favor` e ser `Residente` da região; ii)  `A favor` e ser `Não-Residente`, iii) `Contra` e ser `Residente`; iv)  `Contra` e ser `Não-Residente`. 

Vamos inicialmente representar cada uma das variáveis individualmente por meio de uma tabela de frequência e gráficos de barras dos `r n_amostra` entrevistados.
...
Existem várias formas de gerar um gráfico de barras combinando as duas variáveis. Se quisermos utilizar a própria tabela obtida a partir do comando `table(mun$Opiniao, mun$Moradia)` podemos utilizar o comando `barplot()`. Por outro lado se quisermos utilizar a tabela original de dados (objeto `mun`) podemos fazer uso do `ggplot2`.
...
mun1 = assoc_vq(n = 200, p1 = 0.2, p2 = 0.8, seed = 2)
mun2 = assoc_vq(n = 200, p1 = 0.05, p2 = 0.3, seed = 2)
mun3 = assoc_vq(n = 200, p1 = 0.55, p2 = 0.22, seed = 3)
mun4 = assoc_vq(n = 200, p1 = 0.8, p2 = 0.50, seed = 14)
...
O resultado mostra o o valor de $\chi^2$ calcuado (`X-squared`) e outras duas quantias denominadas de **graus de liberdade** (`df`) e **valor de p** (`p-value`), sobre as quais falaremos na seção sobre Inferência estatística (Capítulos \@ref(popamostra) a \@ref(testet)).

Note que o resultado é diferente do que obtvemos neste capítulo. Isto ocorre pois, por padrão, a função utiliza a chamada **correção de Yates** (<a target="_blank" href="https://en.wikipedia.org/wiki/Yates%27s_correction_for_continuity">veja aqui</a>), em que o $\chi_{Yates}^{2}$ é calculado por:
...
Esta correção será abordada ao falarmos de **testes de hipóteses para variáveis categóricas**. Por hora se quisermos obter exatamente os resultados descrito acima utilizando o a função `chisq.test`, basta fazermos:
