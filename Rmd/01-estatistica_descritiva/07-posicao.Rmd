...
Vamos calcular o índice $Z_i$ para a observação $X_i = `r X[3]`$
...
A interpretação de $Z$ faz sentido quando desejamos posicionar uma determinada observação $X_i$ como função da média e desvio padrões de seu grupo. Veremos nos capítulos \@ref(normdist) e \@ref(tcl) que se uma variável $X$ puder ser descrita adequadamente por uma **Distribuição Normal de probabilidades**, uma regra empírica nos permite determinar qual o percentual das observações está acima e abaixo de alguns limites conhecidos. 
...
Suponha por exemplo que a distribuição de altura de homens adultos siga uma distribuição normal com média $\mu = `r mH`$ cm e desvio padrão de $\sigma = `r mH`$ cm. 
